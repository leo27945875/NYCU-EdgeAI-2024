...
int main(int argc, char** argv) {
  ...
  for (int i = 0; i < num_test_img; i++){
    ...
    const float*  output_result_ptr = output_tensor.const_data_ptr<float>();
    float         max_output_logit  = output_result_ptr[0];
    int32_t       max_output_index  = 0;
    for (int i = 1; i < output_tensor.size(1); i++){
      float logit = output_result_ptr[i];
      if (logit > max_output_logit){
        max_output_logit = logit;
        max_output_index = i;
      }
    }

    auto pred_label = max_output_index;
    
    std::cout << "Output result " << i << " : Label = " << (int32_t)img.label << ", Pred = " << pred_label << std::endl;
